...
class Embed(nn.Module):
    def __init__(self, cfg: TransformerConfig):
        ...
        self.W_E = nn.Parameter(
            torch.randn(cfg.d_embed, cfg.n_vocab) / np.sqrt(cfg.d_embed)
        )
...
class Unembed(nn.Module):
    def __init__(self, cfg: TransformerConfig):
        ...
        self.W_U = nn.Parameter(
            torch.randn(cfg.d_embed, cfg.n_vocab) / np.sqrt(cfg.n_vocab)
        )
...
class PositionEmbed(nn.Module):
    def __init__(self, cfg: TransformerConfig):
        ...
        self.W_pos = nn.Parameter(
            torch.randn(cfg.n_ctx, cfg.d_embed) / np.sqrt(cfg.d_embed)
        )
...
class LayerNorm(nn.Module):
    def __init__(self, cfg: TransformerConfig, epsilon: float = 1e-4):
        super().__init__()
        self.w_ln = nn.Parameter(torch.ones(cfg.d_embed))
        self.b_ln = nn.Parameter(torch.zeros(cfg.d_embed))
...
class Attention(nn.Module):
    def __init__(self, cfg: TransformerConfig):
        ...
        self.W_O = nn.Parameter(
            torch.randn(cfg.d_embed, cfg.d_head * cfg.n_heads) / np.sqrt(cfg.d_embed)
        )
